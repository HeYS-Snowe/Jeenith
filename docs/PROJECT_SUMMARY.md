...
| **紫微斗数** | ziwei | 公历生辰→命身宫+12宫+五行局+14主星+辅星 | ✅ v2 完整（环形命盘 CustomPainter）|
| **奇门遁甲** | qimen | 时辰→阴阳遁+局数+节气+四盘九宫 | ✅ v2 完整（值符值使/八门/九星/八神）|
| **抽签** | chouqian | 60 甲子签，签诗 + 解签 | ✅ 完整 |
| **测字** | cezi | 单字五行 + 笔画断辞 | ✅ 完整 |
...
1. **思源宋体字体**：替换系统默认字体，强化中国风视觉一致性
2. **首次使用引导遮罩**：对高度复杂的术数（大六壬、奇门、紫微）新增首次使用遮罩层或指引
3. **浅色主题细节对齐**：浅色模式下所有 RepaintBoundary 截图颜色准确率审计
4. **紫微/奇门 v3 深化**：紫微辅星细化（火星/铃星/天空/地劫等）；奇门飞盘细化与断辞
5. **历史记录云同步**：可选后端账户体系，跨设备同步历史
6. **更多卜算术**：铁板神数、八字四柱、面相（如果未来需要）
